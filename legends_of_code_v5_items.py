...
while True:
    ...
    if mode == "DRAFT":
        ...
        for card in my_board.values():
            key = card.attack
            while my_board_by_attack.get(key, None):
                key += .1

            my_board_by_attack[key] = card
            debug("key %s" % key)
        ...
        if len(enemy_board) > 0:
            ...
            for enemy in enemy_board.values():
                if "G" in enemy.abilities:
                    key = enemy.defense
                    while target_creatures.get(key, None):
                        key += .1
            ...
            # if there are still targets with g, try to trade
            if len(target_creatures) > 0:
                ...
                for key in sorted_targets.keys():
                    ...
                    for card in sorted_by_attack.values():
                        ...
                        if enemy.will_kill(card):
                            del my_board[card.instance_id]
                            debug("card %s dead" % card.instance_id)
                        else:
                        ...
                        if card.will_kill(enemy):
                            del target_creatures[key]
                            del enemy_board[enemy.instance_id]
                            # enemy is dead, skip so we dont over target
                            debug("enemy %s dead" % enemy.instance_id)
                            break
                        else:
        ...
        if len(my_board) < 6 and len(my_hand) > 0:
            ...
            while can_play_card:
                ...
                for card_no in my_hand.keys():
                    ...
                    if highest_cost < card.cost <= my_mana:
                        highest_cost = card.cost
                        selected_card = card_no
